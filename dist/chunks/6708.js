"use strict";
(self.webpackChunkweb_appedashi = self.webpackChunkweb_appedashi || []).push([
    [6708], {
        16708(e, t, n) {
            n.r(t), n.d(t, {
                default: () => T
            });
            var r = n(15902),
                a = n(70129),
                l = n(51020),
                i = n(56822),
                s = n(96540),
                c = n(31464),
                o = n(15033),
                m = n(1286),
                u = n(12033),
                d = n(68415),
                f = n(27064),
                g = n(43880),
                E = n(52160),
                h = n(74353),
                b = n.n(h),
                w = n(43522),
                p = n.n(w),
                v = n(83826),
                y = n.n(v),
                N = n(5556),
                S = n.n(N),
                x = n(53397),
                I = n(84976),
                C = n(98302),
                J = n(194),
                k = n(91069);
            b().extend(p()), b().extend(y());
            var A = s.memo(function(e) {
                var t, n = e.jam,
                    a = e.className,
                    l = e.refetch,
                    i = s.useRef(null),
                    h = (0, J.a$)().user,
                    w = (0, c.ht)({
                        jamId: n.id,
                        submitterId: null == h ? void 0 : h.id
                    }, {
                        skip: !(null != h && h.id)
                    }),
                    p = w.data,
                    v = w.isLoading,
                    y = (null == p ? void 0 : p.length) > 0,
                    N = s.useMemo(function() {
                        return n.state === m.Uo.CLOSED ? null : (0, k.tF)(n)
                    }, [n]),
                    S = !!N && b().duration(b().utc(N).diff(b().utc())).asHours() < 0,
                    I = s.useState(null === N || S ? "" : b().duration(b().utc(N).diff(b().utc())).format("DD : HH : mm : ss")),
                    A = (0, r.A)(I, 2),
                    D = A[0],
                    U = A[1];
                s.useEffect(function() {
                    return N && (i.current = setInterval(function() {
                            var e = b().duration(b().utc(N).diff(b().utc()));
                            if (e.asSeconds() <= 0) return clearInterval(i.current), i.current = null, void l();
                            U(e.format("DD : HH : mm : ss"))
                        }, 1e3)),
                        function() {
                            i.current && clearInterval(i.current)
                        }
                }, [N]);
                var T = n.type.slice(0, 1).toUpperCase() + n.type.slice(1),
                    _ = n.state === m.Uo.SUBMISSIONS_OPEN && !S,
                    L = y ? "Edit Submission" : "Submit ".concat(T),
                    F = y ? E.faPencil : g.rt,
                    q = s.useMemo(function() {
                        switch (n.state) {
                            case m.Uo.ANNOUNCED:
                                return n.stateChangeDates.submissionsOpened && !S && N ? s.createElement(s.Fragment, null, s.createElement(z, {
                                    className: "tw-mr-3"
                                }, "Submissions Open In"), s.createElement("div", null, D)) : s.createElement(z, null, "Announced");
                            case m.Uo.SUBMISSIONS_OPEN:
                                return n.stateChangeDates.submissionsClosed && N ? s.createElement(s.Fragment, null, s.createElement(z, {
                                    className: "tw-mr-3"
                                }, "Submissions Close In"), s.createElement("div", null, D)) : S ? s.createElement(z, null, "Submissions Closed") : s.createElement(z, null, "Submissions Open");
                            case m.Uo.SUBMISSIONS_CLOSED:
                                return n.stateChangeDates.winnersSelected && !S && N ? s.createElement(s.Fragment, null, s.createElement(z, {
                                    className: "tw-mr-3"
                                }, "Winners Announced In"), s.createElement("div", null, D)) : s.createElement(z, null, "Judging In Progress");
                            case m.Uo.WINNERS_SELECTED:
                                return s.createElement(z, {
                                    className: "tw-mr-3"
                                }, "Winners Selected");
                            default:
                                return ""
                        }
                    }, [n, D, S]);
                return s.createElement(o.fv, {
                    role: "listitem",
                    "aria-label": "Jam Card",
                    className: a
                }, s.createElement(o.N3, null, s.createElement(o.fI, {
                    className: "align-items-center justify-content-between"
                }, s.createElement(O, null, s.createElement("h2", null, n.title), s.createElement("div", {
                    title: "".concat(T, " Jam"),
                    id: "Tooltip-Type-".concat(n.id),
                    className: "tw-ml-3"
                }, s.createElement(o.M2, {
                    icon: "world" === n.type ? f.Bw : d.yV
                })), s.createElement(C.A, {
                    placement: "top",
                    target: "Tooltip-Type-".concat(n.id)
                }, T, " Jam")), s.createElement(R, null, q))), s.createElement(o.BW, null, s.createElement("div", {
                    className: "tw-prose tw-prose-zinc tw-prose-invert mb-2 [&_h3]:after:tw-hidden"
                }, s.createElement(x.Nc, null, null !== (t = n.description) && void 0 !== t ? t : "")), y && s.createElement(o.$T, {
                    type: "neutral",
                    title: "Jam Joined",
                    message: "You have joined this Jam!",
                    hideIcon: !0,
                    slim: !0
                }), s.createElement(o.fI, null, s.createElement(j, {
                    href: n.moreInfo,
                    target: "_blank",
                    className: "tw-mb-2 md:tw-mb-0 tw-mr-0 md:tw-mr-2"
                }, s.createElement("div", null, "Learn More"), s.createElement(o.M2, {
                    className: "tw-ml-3",
                    icon: u.wR
                })), _ && v && s.createElement(o.fy, {
                    height: "38px",
                    className: "flex-1"
                }), _ && !v && s.createElement(M, {
                    to: "".concat(n.id, "/submit"),
                    className: "d-flex flex-row align-items-center justify-content-center"
                }, s.createElement("div", null, L), s.createElement(o.M2, {
                    className: "tw-ml-3",
                    icon: F
                })))))
            });
            A.propTypes = {
                jam: S().shape({
                    id: S().string.isRequired,
                    title: S().string.isRequired,
                    moreInfo: S().string.isRequired
                }).isRequired,
                className: S().string,
                refetch: S().func.isRequired
            };
            const D = A;
            var O = (0, a.A)(o.fI, {
                    target: "e3cywwn4"
                })({
                    name: "1c83l6s",
                    styles: "font-size:1.5rem;font-weight:bold;align-items:center;h2,h3,h4{margin-bottom:0;}"
                }),
                U = {
                    name: "637ilh",
                    styles: "display:flex;align-items:center;justify-content:center;border:#064b5c solid 2px;border-radius:4px;background:#064b5c;color:#6ae3f9;padding:5px 5px;box-sizing:border-box;outline:none!important;flex:1;flex-grow:1;text-decoration:none;&:hover,&:active,&:focus{border-color:#086c84;text-decoration:none;color:#6ae3f9;}"
                },
                j = (0, a.A)("a", {
                    target: "e3cywwn3"
                })(U, ";"),
                M = (0, a.A)(I.N_, {
                    target: "e3cywwn2"
                })(U, " border:#2fb81d07 solid 2px;background:#2fb81d3f;color:white;&:hover,&:active,&:focus{border-color:green;}"),
                R = (0, a.A)(o.fI, {
                    target: "e3cywwn1"
                })({
                    name: "3fn6g6",
                    styles: "align-items:center;font-size:1.5rem;font-weight:bold"
                }),
                z = (0, a.A)("div", {
                    target: "e3cywwn0"
                })({
                    name: "d8vg6u",
                    styles: "font-size:0.9rem;font-weight:normal"
                });
            const T = function() {
                var e = (0, c.xH)(),
                    t = e.data,
                    n = e.isLoading,
                    a = e.isSuccess,
                    i = e.refetch,
                    u = s.useState(!1),
                    d = (0, r.A)(u, 2),
                    f = d[0],
                    g = d[1],
                    E = s.useRef(null);
                s.useEffect(function() {
                    return E.current = setInterval(function() {
                            g(!0), setTimeout(function() {
                                return g(!1)
                            }, 490)
                        }, 5e3),
                        function() {
                            E.current && clearInterval(E.current)
                        }
                }, []);
                var h = s.useMemo(function() {
                        return (null == t ? void 0 : t.filter(function(e) {
                            return e.state !== m.Uo.CLOSED
                        })) || []
                    }, [t]),
                    b = s.useMemo(function() {
                        return (null == t ? void 0 : t.filter(function(e) {
                            return e.state === m.Uo.CLOSED
                        })) || []
                    }, [t]);
                if (n) return s.createElement(o.fy, {
                    width: "100%",
                    height: "200px"
                });
                var w = a && 0 === (null == t ? void 0 : t.length);
                return s.createElement(_, null, s.createElement(o.Qc, null, "VRChat Jams"), s.createElement("h2", null, "VRChat Jams ", s.createElement(q, {
                    isJiggling: f,
                    icon: l.bD
                })), s.createElement(L, {
                    className: "tw-mt-2 tw-mb-3"
                }, s.createElement("p", {
                    className: "mb-0"
                }, "VRChat Jams allow our creative community to flex their development muscles, giving both brand new creators and veterans an opportunity to get their work into the spotlight.", s.createElement("br", null), s.createElement("br", null), "Cook something up by yourself, or meet a few like-minded creators to build something amazing together!", s.createElement("br", null), s.createElement("br", null), "We're excited to see what you build. Good luck — and keep your eye on the clock!")), h.length > 0 && s.createElement(s.Fragment, null, s.createElement("h4", {
                    className: "tw-mb-2"
                }, "Active Jams"), s.createElement(o.fv, {
                    role: "list",
                    "aria-label": "Active Jams"
                }, h.map(function(e) {
                    return s.createElement(D, {
                        refetch: i,
                        className: "tw-mb-2",
                        key: e.id,
                        jam: e
                    })
                }))), b.length > 0 && s.createElement(s.Fragment, null, s.createElement("h4", {
                    className: "tw-mb-4 tw-mt-4"
                }, "Past Jams"), s.createElement(o.fv, {
                    role: "list",
                    "aria-label": "Active Jams"
                }, b.map(function(e) {
                    return s.createElement(D, {
                        refetch: i,
                        className: "tw-mb-2",
                        key: e.id,
                        jam: e
                    })
                }))), w && s.createElement("div", null, "There are no active Jams, come back later!"))
            };
            var _ = (0, a.A)(o.fv, {
                    target: "e1xy0j0z2"
                })({
                    name: "1wafcg9",
                    styles: "max-width:1200px;margin:0 auto"
                }),
                L = (0, a.A)(o.N3, {
                    target: "e1xy0j0z1"
                })({
                    name: "qhoi00",
                    styles: "border-radius:8px;border-width:3px"
                }),
                F = (0, i.i7)({
                    name: "dca843",
                    styles: "0%{transform:rotate(10deg);}25%{transform:rotate(-10deg);}50%{transform:rotate(10deg);}75%{transform:rotate(-10deg);}100%{transform:rotate(10deg);}"
                }),
                q = (0, a.A)(o.M2, {
                    shouldForwardProp: function(e) {
                        return "isJiggling" !== e
                    },
                    target: "e1xy0j0z0"
                })("transform:rotate(10deg);transition:transform 0.5s ease-in-out;animation-name:", F, ";animation-iteration-count:", function(e) {
                    return e.isJiggling ? "infinite" : "0"
                }, ";animation-duration:500ms;animation-timing-function:cubic-bezier(0.175, 0.885, 0.32, 1.275);")
        }
    }
]);
//# sourceMappingURL=000ae2834614ed8123f1391493ecc8a12ee028952056c1ae152e55213fde72b0.js.map